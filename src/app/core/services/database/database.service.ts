...
export class DatabaseService {
    ...
    private readonly oracleQuery = readSQLFile(__dirname, 'sql/oracle.info.sql');
    private readonly postgresQuery = readSQLFile(__dirname, 'sql/postgres.info.sql');
    async getTimestamp(request): Promise<any | null> {
        try {
            ...
            try {
                ...
                const mySQLResult = await this._mySQLService.query(this.mySQLQuery);
                this._logService.debug("MySQL Results", mySQLResult);
                // Get timestamp
                const mySQLQuery = "SELECT now() AS timestamp";
                const [mySQLTimestamp] = await this._mySQLService.query(mySQLQuery);
                // Get MySQL version
                ...
                // Compose Result
                mySQL = {version: mySQLVersion.Value, timestamp: mySQLTimestamp.timestamp, result: mySQLResult};
            } catch (error) {
                mySQL = error.message;
            }
